...
#   We find two co-integrated assets, normally a stock and an ETF index
#  or two stocks in the same industry or any pair that passes the test (cointegration test on the
#  historical data) we set the trigger condition for both stocks and theoretically these two stocks
#  cannot drift too far from each other its like a drunk man with a dog
...
#  sooner or later, the dog would converge to the drunk man nevertheless, the backtest is
#  based on historical datasets in real stock market, market conditions are dynamic.
#   Two assets may seem co-integrated for the past two years they can completely diverge
#  after one company launch a new product or whatsoever (i am talking about nvidia and amd)
#  two gpu companies after bitcoin mining boom and machine learning hype
#  stock price of nvidia went skyrocketing, on the contrary amd didnt change much
#  the cointegrated relationship just broke up so be extremely cautious with cointegration
...
#  Use Engle-Granger two-step method to test co-integration
# the underlying method is straight forward and easy to implement
# the latest statsmodels package should include johansen test which is more common
...
def signal_generation(asset1, asset2, method, bandwidth=250):
    ...
    for i in range(bandwidth, len(signals)):
        ...
        # Cointegration breaks clear existing positions
        ...
        #  Cointegration starts set the trigger conditions this is no forward bias
        ...
        if not prev_status and coint_status:
            ...
            # Normalize the residual to get z stat, z should be a white noise following N(0,1)
            signals['z'].iloc[i:] = (signals['residual'].iloc[i:] - np.mean(model.resid)) / np.std(model.resid)

            # Create thresholds conventionally one sigma is the threshold
    ...
    # Signals only imply holding we take the first order difference to obtain the execution signal
    signals['positions1'] = signals['signals1'].diff()

    # Only need to generate trading signal of one asset the other one should be the opposite direction
